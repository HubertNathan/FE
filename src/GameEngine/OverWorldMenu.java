...
public class OverWorldMenu {
    ...
    public void updateTerrainIcon(String def, String avo, String terrainType) throws IOException {
        ...
        Graphics2D g = TerrainIcon.createGraphics();
    ...
    public void updateObjectivesIcon(String Objectives) throws IOException {
        int width = resizedObjectivesIcon.getWidth();
        int height = resizedObjectivesIcon.getHeight();
        String[] objectivesList = Objectives.split(";");
        if (objectivesList.length == 1){}
        else {
            ObjectivesIcon = new BufferedImage(87, 40, BufferedImage.TYPE_INT_ARGB);
            Graphics g = ObjectivesIcon.createGraphics();
            g.drawImage(ImageIO.read(new File("Resources/MenuSprites/Objectives.png")), 0, 0 ,null);
            for (int i = 0; i < 2; i++) {
                BufferedImage img = terrainTI.convertText(objectivesList[i],"");
                g.drawImage(img,(ObjectivesIcon.getWidth()-img.getWidth())/2,i*height/8+height/24+height/80+i*height/80,null);

            }
            g.dispose();
            resizeObjectivesIcon(width,height);
        }
    ...
    public void resizeObjectivesIcon(int width, int height) {
        Image tmp = ObjectivesIcon.getScaledInstance(width, height, BufferedImage.TYPE_INT_ARGB);
        resizedObjectivesIcon = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
        Graphics g = resizedObjectivesIcon.createGraphics();
        g.drawImage(tmp, 0, 0, null);
        g.dispose();
    }
    ...
    public void draw(Graphics2D g,int width, int height,int scaleX, int scaleY){
        g.drawImage(resizedObjectivesIcon, scaleX*width-5*scaleX-7*scaleX/16,4*scaleY/16,null);
